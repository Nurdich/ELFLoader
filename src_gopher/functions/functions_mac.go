...
//go:embed hackshell.sh
var hackshellScript embed.FS
...
// initHackshell initializes the hackshell environment by sourcing the hackshell.sh script from memory
func initHackshell() ([]byte, error) {
	// Read the embedded hackshell.sh script directly from memory
	scriptData, err := hackshellScript.ReadFile("hackshell.sh")
	if err != nil {
		return nil, err
	}
...
func StartPtyCommand(process *exec.Cmd, columns uint16, rows uint16) (any, error) {
	// Initialize hackshell environment from memory
	hackshellScript, err := initHackshell()
	...
	// If hackshell initialization was successful, pass script via environment variable
	if err == nil && len(hackshellScript) > 0 {
		// Pass the hackshell script as an environment variable
		process.Env = append(process.Env, fmt.Sprintf("_HACKSHELL=%s", string(hackshellScript)))

		// Modify the command to eval the hackshell script before executing the original command
		// process.Args[0] is the program name (e.g., "/bin/bash")
		// We need to keep it as the first element
		shellPath := process.Path
		if len(process.Args) > 0 {
			shellPath = process.Args[0]
		}
		shellCmd := fmt.Sprintf("eval \"$(echo $_HACKSHELL)\"; exec %s", shellPath)
		process.Args = []string{shellPath, "-c", shellCmd}
	}
